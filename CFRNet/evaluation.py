...
from CFRNet.logger import Logger as Log
from CFRNet.loader import *
...
def evaluate_bin_att(predictions, data, i_exp, I_subset=None,
                     compute_policy_curve=False, nn_t=None, nn_c=None,dataset="IHDP"):
    if(dataset.lower().strip() == "ihdp"):
        x = data['x'][:, :, i_exp]
        t = data['t'][:, i_exp]
        e = data['e'][:, i_exp]
        yf = data['yf'][:, i_exp]
...
def evaluate_cont_ate(predictions, data, i_exp, I_subset=None,
    ...
    if(dataset.lower().strip()=="ihdp"):
        ...
        return {'atc_pred': atc_pred, 'bias_ate': bias_ate,
                'ate_pred': ate_pred, 'att_pred': att_pred,
                'bias_att': bias_att, 'bias_atc': bias_atc,
                'rmse_fact': rmse_fact, 'rmse_cfact': rmse_cfact,
                'pehe': pehe, 'rmse_ite': rmse_ite, 'pehe_nn': pehe_appr,"policy_risk":0,"policy_curve":0, "policy_value": 0}
    else:
        x = data['x'][:, :, i_exp]
        t = data['t'][:, i_exp]
        yf = data['yf'][:, i_exp]
        ...
        policy_value, policy_curve = policy_val(t, yf, eff_pred, compute_policy_curve)
        return {'ate_pred': ate_pred, 'att_pred': att_pred,
                'bias_att': 0, 'atc_pred': atc_pred,
                'bias_atc': 0, 'bias_ate': 0,
                'rmse_fact': rmse_fact, 'rmse_cfact': 0,
                'policy_value': policy_value, 'policy_risk': 1 - policy_value,
                'policy_curve': policy_curve,'pehe': 0, 'rmse_ite': 0, 'pehe_nn': 0}
...
def evaluate_result(result, data, validation=False,
                    multiple_exps=False, binary=False,dataset="IHDP"):
        ...
        for i_rep in range(n_rep):
            ...
            if binary:
                eval_result = evaluate_bin_att(predictions[:, :, i_rep, i_out],
                                               data, i_exp, I_valid_rep, compute_policy_curve, nn_t=nn_t, nn_c=nn_c,dataset=dataset)
            else:
    ...
    for result in results:
        ...
        try:
            eval_train = evaluate_result(result['train'], data_train,
                                         validation=False, multiple_exps=multiple_exps, binary=binary, dataset=dataset)

            eval_valid = evaluate_result(result['train'], data_train,
                                         validation=True, multiple_exps=multiple_exps, binary=binary,dataset=dataset)
