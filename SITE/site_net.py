...
class site_net(object):
    def __init__(self, x, t, y_ , p_t, FLAGS, r_lambda, do_in, do_out, dims, three_pairs, three_pairs_simi, r_mid_point_mini, r_pddm):
        ...
        if FLAGS.nonlin.lower() == 'elu':
            self.nonlin = tf.compat.v1.compat.v1.nn.elu
        else:
            self.nonlin = tf.compat.v1.compat.v1.nn.relu
    ...
    def _create_variable(self, var, name):
        ...
        var = tf.compat.v1.compat.v1.get_variable(name=name, initializer=var)
    ...
    def _create_variable_with_weight_decay(self, initializer, name, wd):
        ...
        self.wd_loss += wd*tf.compat.v1.compat.v1.nn.l2_loss(var)
        return var
    ...
    def _build_graph(self, x, t, y_ , p_t, FLAGS, r_lambda, do_in, do_out, dims, three_pairs,  three_pairs_simi, r_mid_point_mini, r_pddm):
        ...
        for i in range(0, FLAGS.n_in):
            if i==0:
                ...
                if FLAGS.varsel:
                    weights_in.append(tf.compat.v1.compat.v1.Variable(1.0/dim_input*tf.compat.v1.compat.v1.ones([dim_input])))
                else:
                    weights_in.append(tf.compat.v1.compat.v1.Variable(tf.compat.v1.compat.v1.random_normal([dim_input, dim_in], stddev=FLAGS.weight_init/np.sqrt(dim_input))))
            else:
                weights_in.append(tf.compat.v1.compat.v1.Variable(tf.compat.v1.compat.v1.random_normal([dim_in,dim_in], stddev=FLAGS.weight_init/np.sqrt(dim_in))))

            ''' If using variable selection, first layer is just rescaling'''
            if FLAGS.varsel and i==0:
                biases_in.append([])
                h_in.append(tf.compat.v1.compat.v1.mul(h_in[i],weights_in[i]))
                h_in_batch.append(tf.compat.v1.compat.v1.mul(h_in_batch[i],weights_in[i]))
            else:
                biases_in.append(tf.compat.v1.compat.v1.Variable(tf.compat.v1.compat.v1.zeros([1,dim_in])))
                z = tf.compat.v1.compat.v1.matmul(h_in[i], weights_in[i]) + biases_in[i]
                z_batch = tf.compat.v1.compat.v1.matmul(h_in_batch[i], weights_in[i]) + biases_in[i]

                if FLAGS.batch_norm:
                    batch_mean, batch_var = tf.compat.v1.compat.v1.nn.moments(z, [0])
                    batch_mean_batch, batch_var_batch = tf.compat.v1.compat.v1.nn.moments(z_batch, [0])
                    if FLAGS.normalization == 'bn_fixed':
                        z = tf.compat.v1.compat.v1.nn.batch_normalization(z, batch_mean, batch_var, 0, 1, 1e-3)
                        z_batch = tf.compat.v1.compat.v1.nn.batch_normalization(z_batch, batch_mean_batch, batch_var_batch, 0, 1, 1e-3)
                    else:
                        bn_biases.append(tf.compat.v1.compat.v1.Variable(tf.compat.v1.compat.v1.zeros([dim_in])))
                        bn_scales.append(tf.compat.v1.compat.v1.Variable(tf.compat.v1.compat.v1.ones([dim_in])))
                        bn_biases_batch.append(tf.compat.v1.compat.v1.Variable(tf.compat.v1.compat.v1.zeros([dim_in])))
                        bn_scales_batch.append(tf.compat.v1.compat.v1.Variable(tf.compat.v1.compat.v1.ones([dim_in])))
                        z = tf.compat.v1.compat.v1.nn.batch_normalization(z, batch_mean, batch_var, bn_biases[-1], bn_scales[-1], 1e-3)
                        z_batch = tf.compat.v1.compat.v1.nn.batch_normalization(z_batch, batch_mean_batch, batch_var_batch, bn_biases_batch[-1], bn_scales_batch[-1], 1e-3)

                h_in.append(self.nonlin(z))
                h_in_batch.append(self.nonlin(z_batch))
                h_in[i+1] = tf.nn.dropout(h_in[i+1], 1-do_in)
                h_in_batch[i + 1] = tf.nn.dropout(h_in_batch[i + 1], 1-do_in)
        ...
        if FLAGS.normalization == 'divide':
            h_rep_norm = h_rep / safe_sqrt(tf.reduce_sum(tf.compat.v1.square(h_rep), axis=1, keepdims=True))
            h_rep_norm_batch = h_rep_batch / safe_sqrt(tf.reduce_sum(tf.compat.v1.square(h_rep_batch), axis=1, keepdims=True))
        else:
        ...
        x_i = tf.compat.v1.slice(h_rep_norm_batch, [0, 0], [1, dim_in])
        x_j = tf.compat.v1.slice(h_rep_norm_batch, [1, 0], [1, dim_in])
        x_k = tf.compat.v1.slice(h_rep_norm_batch, [2, 0], [1, dim_in])
        x_l = tf.compat.v1.slice(h_rep_norm_batch, [3, 0], [1, dim_in])
        x_m = tf.compat.v1.slice(h_rep_norm_batch, [4, 0], [1, dim_in])
        x_n = tf.compat.v1.slice(h_rep_norm_batch, [5, 0], [1, dim_in])
        with tf.compat.v1.variable_scope('pddm') as scope:
        ...
        '''pddm loss'''
        simi_kl = tf.compat.v1.slice(three_pairs_simi, [0, 0], [1, 1])
        simi_mn = tf.compat.v1.slice(three_pairs_simi, [1, 0], [1, 1])
        simi_km = tf.compat.v1.slice(three_pairs_simi, [2, 0], [1, 1])
        simi_ik = tf.compat.v1.slice(three_pairs_simi, [3, 0], [1, 1])
        simi_jm = tf.compat.v1.slice(three_pairs_simi, [4, 0], [1, 1])
        pddm_loss= tf.compat.v1.reduce_sum(tf.compat.v1.square(simi_kl - s_kl) + tf.compat.v1.square(simi_mn - s_mn)
                                 + tf.compat.v1.square(simi_km - s_km) + tf.compat.v1.square(simi_ik - s_ik)
                                 + tf.compat.v1.square(simi_jm - s_jm))


        '''mid_point distance minimization'''
        mid_jk = (x_j + x_k) / 2.0
        mid_im = (x_i + x_m) / 2.0
        mid_distance = tf.compat.v1.reduce_sum(tf.compat.v1.square(mid_jk - mid_im), [0, 1])
    ...
    def _build_output(self, h_input, dim_in, dim_out, do_out, FLAGS):
        ...
        for i in range(0, FLAGS.n_out):
            ...
            h_out[i+1] = tf.nn.dropout(h_out[i+1], 1-do_out)
        ...
        if FLAGS.varsel or FLAGS.n_out == 0:
            self.wd_loss += tf.compat.v1.nn.l2_loss(tf.compat.v1.slice(weights_pred,[0,0],[dim_out-1,1])) #don't penalize treatment coefficient
        else:
    ...
    def pddm(self, x_i, x_j, dim_in, dim_pddm, dim_c,dim_s, FLAGS):
        ...
        u = tf.compat.v1.abs(x_i-x_j)
        v = (x_i+x_j)/2.0
        b_u = []
        b_v = []
        b_c = []
        b_s = []
        b_u.append(tf.compat.v1.Variable(tf.compat.v1.zeros([1, dim_pddm])))
        b_v.append(tf.compat.v1.Variable(tf.compat.v1.zeros([1, dim_pddm])))
        b_c.append(tf.compat.v1.Variable(tf.compat.v1.zeros([1, dim_c])))
        b_s.append(tf.compat.v1.Variable(tf.compat.v1.zeros([1, dim_s])))
        w_u = self._create_variable_with_weight_decay(
            tf.compat.v1.random_normal([dim_in, dim_pddm],
        ...
        w_c = self._create_variable_with_weight_decay(
            tf.compat.v1.random_normal([concate_dim, dim_c],
                             stddev=FLAGS.weight_init / np.sqrt(concate_dim)),
            'w_c', 1.0)
        weights_pddm.append(w_c)

        w_s = self._create_variable_with_weight_decay(
            tf.compat.v1.random_normal([dim_c, dim_s],
                             stddev=FLAGS.weight_init / np.sqrt(dim_c)),
            'w_s', 1.0)
        weights_pddm.append(w_s)

        u_1 = tf.compat.v1.matmul(u, w_u) + b_u
        v_1 = tf.compat.v1.matmul(v, w_v) + b_v
        u_1 = self.nonlin(u_1)
        v_1 = self.nonlin(v_1)
        u_1 = tf.nn.l2_normalize(u_1, axis = 0)
        v_1 = tf.nn.l2_normalize(v_1, axis = 0)
        c = tf.compat.v1.concat([u_1, v_1], 1)
        c = tf.compat.v1.reshape(c, [1,concate_dim])
        c = tf.compat.v1.matmul(c, w_c) + b_c
        c = self.nonlin(c)
        c = tf.compat.v1.reshape(c, [1, dim_c])
        s = tf.compat.v1.matmul(c, w_s) + b_s
    ...
    def _build_output_graph(self, rep, t, dim_in, dim_out, do_out, FLAGS):
        ...
        if FLAGS.split_output:
            ...
            i0 = tf.cast(tf.compat.v1.where(t < 1)[:,0],tf.int32)
            i1 = tf.cast(tf.compat.v1.where(t > 0)[:,0],tf.int32)

            rep0 = tf.compat.v1.gather(rep, i0)
            rep1 = tf.compat.v1.gather(rep, i1)
            with tf.compat.v1.variable_scope("control") as scope:

                y0, weights_out0, weights_pred0 = self._build_output(rep0, dim_in, dim_out, do_out, FLAGS)

            with tf.compat.v1.variable_scope("treated") as scope:
            ...
            h_input = tf.compat.v1.concat([rep, t],1)
