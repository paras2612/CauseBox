...
import SITE.evaluate as ev
import SITE.site_net as site
...
tf.compat.v1.app.flags.DEFINE_integer('rep_weight_decay', 1, """Whether to penalize representation layers with weight decay""")
...
tf.compat.v1.app.flags.DEFINE_integer('experiments', 1, """Number of experiments. """)
...
tf.compat.v1.app.flags.DEFINE_string('outdir', '../results/ihdp/', """Output directory. """)
tf.compat.v1.app.flags.DEFINE_string('datadir', '../data/', """Data directory. """)
tf.compat.v1.app.flags.DEFINE_string('dataform', 'ihdp_npci_1-100.train.npz', """Training data filename form. """)
tf.compat.v1.app.flags.DEFINE_string('data_test', '', """Test data filename form. """)
...
tf.compat.v1.app.flags.DEFINE_string('optimizer', 'RMSProp', """Which optimizer to use. (RMSProp/Adagrad/GradientDescent/Adam)""")
tf.compat.v1.app.flags.DEFINE_string('imb_fun', 'mmd_lin', """Which imbalance penalty to use (mmd_lin/mmd_rbf/mmd2_lin/mmd2_rbf/lindisc/wass). """)
tf.compat.v1.app.flags.DEFINE_integer('output_csv',0,"""Whether to save a CSV file with the results_try1""")
...
tf.compat.v1.app.flags.DEFINE_float('val_part', 0, """Validation part. """)
...
tf.compat.v1.app.flags.DEFINE_float('p_pddm', 1.0, """PDDM unit parameter """)
tf.compat.v1.app.flags.DEFINE_float('p_mid_point_mini', 1.0, """Mid point distance minimization parameter """)
tf.compat.v1.app.flags.DEFINE_float('dim_pddm', 100.0, """Dimension in PDDM fist layer """)
tf.compat.v1.app.flags.DEFINE_float('dim_c', 100.0, """Dimension in PDDM unit for c """)
tf.compat.v1.app.flags.DEFINE_float('dim_s', 100.0, """Dimension in PDDM unit for s """)
tf.compat.v1.app.flags.DEFINE_string('propensity_dir','./propensity_score/ihdp_propensity_model.sav', """Dir where the propensity model is saved""" )
tf.compat.v1.app.flags.DEFINE_boolean('equal_sample', 0, """Whether to fectch equal number of samples with different labels. """)
tf.compat.v1.app.flags.DEFINE_string('dataset', "IHDP", """Which dataset to use """)
...
def run(outdir):
    ...
    three_pairs = tf.compat.v1.placeholder("float", shape=[6, D['dim']], name='three_pairs')

    # the similarity score of selected three pairs (ground truth similarity)
    three_pairs_simi = tf.compat.v1.placeholder("float", shape = [5,1], name = 'three_pairs_simi')
    ...
    n_train_graph = tf.compat.v1.placeholder("float", name='n_train_graph')

    # the hyperparameter for MPDM loss
    r_mid_point_mini = tf.compat.v1.placeholder("float", name='r_mid_point_mini')

    # the hyperparameter for PDDM loss
    r_pddm = tf.compat.v1.placeholder("float", name='r_pddm')
    ...
    for i_exp in range(1,n_experiments+1):
        print('Training on experiment %d/%d...' % (i_exp, n_experiments))
        print("*" * 100)
