...
import DRN.dr_cfr as cfr
from DRN.evaluate import evaluate
...
tf.compat.v1.app.flags.DEFINE_float('p_lambda', 1e-4, """Weight decay regularization parameter. """)
tf.compat.v1.app.flags.DEFINE_float('p_beta', 1e-3, """Weight decay regularization parameter. """)
tf.compat.v1.app.flags.DEFINE_integer('rep_weight_decay', 1,
                            """Whether to penalize representation layers with weight decay""")
tf.compat.v1.app.flags.DEFINE_float('dropout_in', 0.9, """Input layers dropout keep rate. """)
tf.compat.v1.app.flags.DEFINE_float('dropout_out', 0.9, """Output layers dropout keep rate. """)
tf.compat.v1.app.flags.DEFINE_string('nonlin', 'relu', """Kind of non-linearity. Default relu. """)
tf.compat.v1.app.flags.DEFINE_float('lrate', 0.05, """Learning rate. """)
tf.compat.v1.app.flags.DEFINE_float('decay', 0.5, """RMSProp decay. """)
tf.compat.v1.app.flags.DEFINE_integer('batch_size', 100, """Batch size. """)
tf.compat.v1.app.flags.DEFINE_integer('dim_in', 100, """Pre-representation layer dimensions. """)
tf.compat.v1.app.flags.DEFINE_integer('dim_out', 100, """Post-representation layer dimensions. """)
tf.compat.v1.app.flags.DEFINE_integer('batch_norm', 0, """Whether to use batch normalization. """)
tf.compat.v1.app.flags.DEFINE_string('normalization', 'none',
...
tf.compat.v1.app.flags.DEFINE_integer('experiments', 5, """Number of experiments. """)
tf.compat.v1.app.flags.DEFINE_integer('iterations', 2000, """Number of iterations. """)
tf.compat.v1.app.flags.DEFINE_float('weight_init', 0.01, """Weight initialization scale. """)
tf.compat.v1.app.flags.DEFINE_float('lrate_decay', 0.95, """Decay of learning rate every 100 iterations """)
tf.compat.v1.app.flags.DEFINE_integer('wass_iterations', 20, """Number of iterations in Wasserstein computation. """)
tf.compat.v1.app.flags.DEFINE_float('wass_lambda', 1, """Wasserstein lambda. """)
tf.compat.v1.app.flags.DEFINE_integer('wass_bpt', 0, """Backprop through T matrix? """)
tf.compat.v1.app.flags.DEFINE_integer('varsel', 0, """Whether the first layer performs variable selection. """)
tf.compat.v1.app.flags.DEFINE_string('outdir', r'C:/Users/paras/PycharmProjects/pythonProject/CFRNet/Results/',
                           """Output directory. """)
tf.compat.v1.app.flags.DEFINE_string('datadir', r'C:/Users/paras/PycharmProjects/pythonProject/CFRNet/Dataset/',
                           """Data directory. """)
tf.compat.v1.app.flags.DEFINE_string('dataform', 'ihdp_npci_1-1000.train.npz/ihdp_npci_1-1000.train.npz',
                           """Training data filename form. """)
tf.compat.v1.app.flags.DEFINE_string('data_test', '/ihdp_npci_1-1000.test.npz/ihdp_npci_1-1000.test.npz',
                           """Test data filename form. """)
...
tf.compat.v1.app.flags.DEFINE_integer('use_p_correction', 1, """Whether to use population size p(t) in mmd/disc/wass.""")
tf.compat.v1.app.flags.DEFINE_string('optimizer', 'RMSProp',
                           """Which optimizer to use. (RMSProp/Adagrad/GradientDescent/Adam)""")
tf.compat.v1.app.flags.DEFINE_string('imb_fun', 'mmd_lin',
                           """Which imbalance penalty to use (mmd_lin/mmd_rbf/mmd2_lin/mmd2_rbf/lindisc/wass). """)
tf.compat.v1.app.flags.DEFINE_integer('output_csv', 0, """Whether to save a CSV file with the results""")
tf.compat.v1.app.flags.DEFINE_integer('output_delay', 100, """Number of iterations between log/loss outputs. """)
tf.compat.v1.app.flags.DEFINE_integer('pred_output_delay', -1,
...
tf.compat.v1.app.flags.DEFINE_integer('save_rep', 0, """Save representations after training. """)
tf.compat.v1.app.flags.DEFINE_float('val_part', 0.3, """Validation part. """)
tf.compat.v1.app.flags.DEFINE_boolean('split_output', 0,
                            """Whether to split output layers between treated and control. """)
tf.compat.v1.app.flags.DEFINE_boolean('reweight_sample', 1,
                            """Whether to reweight sample for prediction loss with average treatment probability. """)
tf.compat.v1.app.flags.DEFINE_string('dataset', "IHDP", """Specify which dataset model needs to be evaluated on """)
...
def train(CFR, sess, train_step, D, I_valid, D_test, logfile, i_exp):
    ...
    dict_factual = {CFR.x: D['x'][I_train,:], CFR.t: D['t'][I_train,:], CFR.y_: D['yf'][I_train,:],CFR.do_in: 1.0, CFR.do_out: 1.0, CFR.r_alpha: FLAGS.p_alpha,CFR.r_lambda: FLAGS.p_lambda, CFR.r_beta: FLAGS.p_beta,CFR.p_t: p_treated}

    if FLAGS.val_part > 0:
        dict_valid = {CFR.x: D['x'][I_valid,:], CFR.t: D['t'][I_valid,:], CFR.y_: D['yf'][I_valid,:], \
          CFR.do_in: 1.0, CFR.do_out: 1.0, CFR.r_alpha: FLAGS.p_alpha, \
          CFR.r_lambda: FLAGS.p_lambda,CFR.r_beta: FLAGS.p_beta, CFR.p_t: p_treated}
    ...
    for i in range(FLAGS.iterations):
        ...
        if not objnan:
            sess.run(train_step, feed_dict={CFR.x: x_batch, CFR.t: t_batch, \
                ...
                CFR.r_alpha: FLAGS.p_alpha, CFR.r_lambda: FLAGS.p_lambda, CFR.r_beta: FLAGS.p_beta, CFR.p_t: p_treated})
...
def run(outdir):
    ...
    x  = tf.compat.v1.placeholder("float", shape=[None, D['dim']], name='x') # Features
    ...
    r_beta = tf.compat.v1.placeholder("float", name='r_beta')
    ...
    CFR = cfr.dr_cfr(x, t, y_, dims, do_in, do_out,p, r_alpha, r_lambda, r_beta, FLAGS)


    ''' Set up optimizer '''
    global_step = tf.compat.v1.Variable(0, trainable=False)
    lr = tf.compat.v1.train.exponential_decay(FLAGS.lrate, global_step, \
    ...
    #capped_gvs = [(tf.compat.v1.clip_by_value(grad, -1.0, 1.0), var) for grad, var in gvs]
...
if __name__=="__main__":
    ...
    outdir = os.getcwd() + '\\CFRNet\\Results\\' + sys.argv[-1].lower().strip() + '\\results_' + timestamp + '\\'
    os.makedirs(outdir)
    try:
        run(outdir)
