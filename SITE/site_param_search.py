...
def run(cfg_file, num_runs):
    ...
    data_path = "D:\\PycharmProjects\\pythonProject\\SITE\\" + configs['datadir'][0] + configs['dataform'][0]
    print(data_path)
    propensity_dir = configs['propensity_dir'][0]
    call('python D:\PycharmProjects\pythonProject\SITE\propensity_score_calculation.py %s %s' % (data_path, propensity_dir), shell=True)
    ...
    for i in range(num_runs):
        ...
        call('python D:\PycharmProjects\pythonProject\SITE\site_net_train.py %s' % flags, shell=True)
