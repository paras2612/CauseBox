...
def validation_split_equal(D_exp, val_fraction):
    ...
    if val_fraction > 0:
        ...
        n_train = n-n_valid
        I = np.random.permutation(range(0,n))
        I_train = np.array(I[:n_train], dtype=int)
        I_valid = np.array(I[n_train:], dtype=int)
    else:
        I_train = np.array(range(n), dtype=int)
        I_valid = np.array([], dtype=int)
